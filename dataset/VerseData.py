...
class Dataset(object):
    def __init__(self,
        ...
        :param image_size: 网络输入图像的大小。
        :param image_spacing: 网络输入图像的间距。
        :param base_folder: 数据集基本文件夹。
        :param image_base_folder:
        :param setup_base_folder:
        :param cv: 交叉验证索引（0，1，2），如果进行完整的训练/测试，则为“ train_all”，仅在推理模式下为“推理”。
        :param input_gaussian_sigma: 输入平滑的Sigma值。
        :param label_gaussian_sigma: 标签平滑的Sigma值。
        :param load_spine_landmarks: 如果为true，则加载spine地标文件。
        :param load_spine_bbs:
        :param generate_labels: 如果为true，则生成多标签椎骨分割。
        :param generate_heatmaps: 如果为true，则生成椎骨热图。
        :param generate_landmarks: 如果为true，则生成地标坐标。
        :param generate_single_vertebrae_heatmap: 如果为true，则生成单个椎骨热图。
        :param generate_single_vertebrae: 如果为true，则生成单个椎骨分割。
        :param generate_spine_heatmap: 如果为true，则生成书脊热图。
        :param generate_landmark_mask: 如果为true，则在体积的顶部和底部生成具有25 mm黑色的地标掩模。
        :param crop_image_top_bottom:
        :param translate_by_random_factor: 如果为true，则在不适合image_size的情况下对输入量进行随机裁剪。
        :param translate_to_center_landmarks: 如果为true，则将输入图像转换为地标中心。
        :param use_variable_image_size:
        :param valid_output_sizes_x:
        :param valid_output_sizes_y:
        :param valid_output_sizes_z:
        :param random_translation: 用于训练数据扩充的随机翻译量。
        :param random_scale: 用于训练数据扩充的随机量。
        :param random_rotate: 用于训练数据扩充的随机旋转量。
        :param random_deformation: 用于训练数据扩充的随机弹性变形量。
        :param random_intensity_shift: 用于训练数据增强的随机强度偏移量。
        :param random_intensity_scale: 用于训练数据扩充的随机强度标度的数量。
        :param normalize_zero_mean_unit_variance:
        :param random_translation_single_landmark: 生成单个地标输出时，地标的随机翻译量。
        :param random_flip:
        :param resample_iterator:
        :param num_landmarks: 地标数量。
        :param num_labels: 标签数。
        :param heatmap_sigma: 用于生成地标热图的Sigma值。
        :param single_heatmap_sigma:
        :param spine_heatmap_sigma: 生成脊柱热图的Sigma值。
        :param crop_randomly_smaller:
        :param data_format: 'channels_first'或'channels_last'
        :param output_image_type:
        :param save_debug_images: 如果为true，则将生成的图像保存到磁盘。
        '''
        ...
        if self.cv in [0, 1, 2]:
            ...
            self.valid_landmarks_file = os.path.join(self.setup_base_folder,
                                                     'vertebrae_localization/valid_landmarks.csv')
    ...
    def datasources(self, iterator, image_cached, labels_cached, image_preprocessing, cache_size):
        ...
        if self.generate_landmarks or self.generate_heatmaps or self.generate_spine_heatmap or self.generate_single_vertebrae or self.generate_single_vertebrae_heatmap or (
            ...
            datasources_dict['landmarks_bb_start'] = LambdaNode(lambda x: x[0], name='landmarks_bb_start',
                                                                parents=[datasources_dict['landmarks_bb']])
            datasources_dict['landmarks_bb_extent'] = LambdaNode(lambda x: x[1], name='landmarks_bb_extent',
                                                                 parents=[datasources_dict['landmarks_bb']])
        if self.load_spine_landmarks:
            datasources_dict['spine_landmarks'] = LandmarkDataSource(self.spine_landmarks_file, 1, self.dim,
        ...
        if self.generate_single_vertebrae:
            if self.generate_labels:
                ...
                labels_unsmoothed = ImageGenerator(self.dim,
                                                   ...
                                                   kwparents=kwparents)
                generators_dict['single_label'] = LambdaNode(
                    lambda id_dict, labels: self.split_and_smooth_single_label(labels, int(id_dict['landmark_id'])),
                    name='single_label',
    ...
    def landmark_based_crop(self, image, landmarks):
        ...
        if not landmarks[0].is_valid:
            upper_crop = np.maximum(upper_crop, (image.GetSpacing()[2] * image.GetSize()[2]) - (
                        max_coords[2] + median_distance * 0.5))
    ...
    def dataset_train(self):
        ...
        if self.use_variable_image_size:
            image_size = ImageSizeGenerator(self.dim, [None] * 3, self.image_spacing,
                                            valid_output_sizes=[self.valid_output_sizes_x, self.valid_output_sizes_y,
                                                                self.valid_output_sizes_z], name='output_size',
                                            kwparents={'extent': sources['landmarks_bb_extent']})
    ...
    def dataset_val(self):
        ...
        if self.use_variable_image_size:
            if self.load_spine_bbs:
                image_size = ImageSizeGenerator(self.dim, [None] * 3, self.image_spacing,
                                                valid_output_sizes=[[32, 64, 96, 128], [32, 64, 96, 128],
                                                                    [32 + i * 32 for i in range(20)]],
                                                name='output_size',
                                                kwparents={'extent': sources['landmarks_bb_extent']})
            else:
                image_size = ImageSizeGenerator(self.dim, [None] * 3, self.image_spacing,
                                                valid_output_sizes=[[32, 64, 96, 128], [32, 64, 96, 128],
                                                                    [32 + i * 32 for i in range(20)]],
                                                name='output_size', kwparents={'image': sources['image']})
        else:
            image_size = LambdaNode(lambda: self.image_size, name='output_size')
